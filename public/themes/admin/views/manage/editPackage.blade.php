<form class="form-horizontal addForm" role="form" enctype="multipart/form-data" method="post" action="/manage/editPackage/{{$packageInfo['id']}}">
    ...
    <div class="g-backrealdetails clearfix bor-border interface">
            ...
            @if(!empty($privileges))
                <div class="col-sm-8">
                    @foreach($privileges as $pv)
                        <label>
                            <input type="checkbox" class="drag-tabinp" value="{{$pv['id']}}" @if(in_array($pv['id'],$packageInfo['privileges'])) checked="checked" name="privileges[]" @endif data-code="{{$pv['code']}}">
        ...
        <div class="form-group interface-bottom col-xs-12" id="TASK_WORK" style="@if(in_array('TASK_WORK',array_keys($packageInfo['more_rule'])))display: block; @else display: none; @endif">
            <label class="col-sm-1 control-label no-padding-right" for="form-field-1">竞标次数/天</label>

            <div class="col-sm-8">
                <input type="text" name="TASK_WORK" value="@if(in_array('TASK_WORK',array_keys($packageInfo['more_rule']))){{$packageInfo['more_rule']['TASK_WORK']['rule']}}@endif">
            </div>

        </div>
        <div class="form-group interface-bottom col-xs-12" id="MOST_TASK_BOUNTY" style="@if(in_array('MOST_TASK_BOUNTY',array_keys($packageInfo['more_rule'])))display: block; @else display: none; @endif">
            <label class="col-sm-1 control-label no-padding-right" for="form-field-1">可竞标最高金额</label>

            <div class="col-sm-8">
                <input type="text" name="MOST_TASK_BOUNTY" value="@if(in_array('MOST_TASK_BOUNTY',array_keys($packageInfo['more_rule']))){{$packageInfo['more_rule']['MOST_TASK_BOUNTY']['rule']}}@endif">
            </div>

        </div>
        <div class="form-group interface-bottom col-xs-12" id="SKILL_TAGS_NUM" style="@if(in_array('SKILL_TAGS_NUM',array_keys($packageInfo['more_rule'])))display: block; @else display: none; @endif">
            <label class="col-sm-1 control-label no-padding-right" for="form-field-1">技能标签数量</label>

            <div class="col-sm-8">
                <input type="text" name="SKILL_TAGS_NUM" value="@if(in_array('SKILL_TAGS_NUM',array_keys($packageInfo['more_rule']))){{$packageInfo['more_rule']['SKILL_TAGS_NUM']['rule']}}@endif">
            </div>

        </div>
        <div class="form-group interface-bottom col-xs-12" id="SERVICE_OFF" style="@if(in_array('SERVICE_OFF',array_keys($packageInfo['more_rule'])))display: block; @else display: none; @endif">
            <label class="col-sm-1 control-label no-padding-right" for="form-field-1">增值工具折扣%</label>

            <div class="col-sm-8">
                <input type="text" name="SERVICE_OFF" value="@if(in_array('SERVICE_OFF',array_keys($packageInfo['more_rule']))){{$packageInfo['more_rule']['SERVICE_OFF']['rule']}}@endif">
            </div>
