    ...
    <form action="/manage/handleRights" method="post">
        ...
        <div class="bankAuth-bottom clearfix col-xs-12">
            ...
            <div class="col-md-11">
                ...
                @if($report['status']==0)
                    @if($report['role']==0)
                        <p>{!! Theme::get('site_config')['site_employer'] ? Theme::get('site_config')['site_employer'] : '雇主' !!} ({{ $to_user['nickname'] }}) 获得 <input type="text" name="owner_bounty"/>元，{!! Theme::get('site_config')['site_employee'] ? Theme::get('site_config')['site_employee'] : '威客' !!} ({{ $from_user['nickname'] }}) 获得 <input type="text" name="worker_bounty" />元</p>
                    @endif
                    @if($report['role']==1)
                        <p>{!! Theme::get('site_config')['site_employer'] ? Theme::get('site_config')['site_employer'] : '雇主' !!} ({{ $from_user['nickname'] }}) 获得 <input type="text" name="owner_bounty" />元，{!! Theme::get('site_config')['site_employee'] ? Theme::get('site_config')['site_employee'] : '威客' !!} ({{ $to_user['nickname'] }}) 获得 <input type="text" name="worker_bounty"  />元</p>
                    @endif
                @endif
            </div>
        </div>
