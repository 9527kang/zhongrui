            ...
            <div data-target="#step-container" class="row-fluid" id="fuelux-wizard">
                <ul class="wizard-steps poster-time">
                    <li class="active text-left" data-target="#step1">
                        <span class="step">
                            <img src="/themes/default/assets/images/describe_icon.png" alt="">
                        </span>
                        <span class="title h6 p-space">需求描述</span>
                    </li>
                    <li data-target="#step2">
                        <span class="step">
                            <img src="/themes/default/assets/images/deal_icon.png" alt="" style="position: relative;top:-3px;">
                        </span>
                        <span class="title h6 p-space">交易模式</span>
                    </li>
                    <li data-target="#step3">
                        <span class="step">
                            <img src="/themes/default/assets/images/issue_icon.png" alt="" style="position: relative;top:-2px;">
                        </span>
                        <span class="title h6 p-space">确认需求，托管赏金</span>
                    </li>
                    <li data-target="#step4 text-right">
                        <span class="step">
                            <img src="/themes/default/assets/images/stepStatus_icon.png" alt="">
                        </span>
            ...
            <form action="/task/createTask" method="post" id="form" enctype="multipart/form-data">
                ...
                <div class="row">
                            ...
                            <li>
                                ...
                                <div id="demo" class="collapse in">
                                    ...
                                    <div class="form-group task-phone">
                                        ...
                                        <input type="text" class="form-control task-input" id="name"  name="phone"  value="@if($type == 0){{trim($task->phone)}} @endif" placeholder="请输入手机号" datatype="n" nullmsg="请填写手机号！" errormsg="手机号错误！">
                                    ...
                                    <div class="task-select1" id="gd"  style="margin-bottom: 0px;">
                                        ...
                                        <div style="float:left;">
                                            <select {{--multiple="" class="chosen-select tag-input-style"--}} name="cate_pid"  {{--data-placeholder="选择一下您的标签吧" --}}style="display:block;" onchange="getCate(this.value, 'secondCate')">
                                                @foreach($category_all as $v)
                                                    <option value="{{ $v['id'] }}" cate-id="{{ $v['id'] }}"{{-- class="chooseCate"--}} {{ ($pid==$v['id'])?'selected':'' }}>
                                                        {{ $v['name'] }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div style="float:left;">
                                            <select name="cate_id" id="secondCate" onchange="chooseCateId(this.value)">
                                                <option value="">请选择</option>
                                                @if(!empty($category_second))
                                                    @foreach($category_second as $item)
                                                        <option value="{!! $item['id'] !!}" {{ ($task['cate_id']==$item['id'])?'selected':'' }}>
                                                            {!! $item['name'] !!}
                                                        </option>
                                                    @endforeach
                                                @endif
                                    ...
                                    <div class="task-bar">
                                        <span class="text-size14" style="text-align:left;margin-top:57px;display: block;margin-bottom:7px;">地域：</span>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="row">
                                            <div class="col-sm-12">
                                                <input type="text" class="col-xs-10 col-sm-5" name="title" value="{!! old('title') !!}{{ !empty($_GET['title'])?$_GET['title']:'' }}{{ $task->title }}" id="form-input-readonly" placeholder="一句话描述您的需求，XX餐饮公司VI设计" datatype="*5-50" nullmsg="请填写标题！">
                            ...
                            <li>
                                ...
                                <div id="demol" class="panel-group">
                                    ...
                                    <div class="panel">
                                        ...
                                        <div class="checkbox">
                                            <label class="z-check z-check-validform" data-toggle="collapse" data-parent="#demol" href="#ck">
                                                <input  type="radio" class="ace" name="type_id" @if($task['type_id'] == $type_v['id'])checked="checked" @endif datatype="*" nullmsg="请选择交易模式" value="{{$type_v['id']}}" data-values="{{$type_v['alias']}}"/>
                                                <span class="lbl text-blod text-size16 pull-left"> 悬赏模式</span>
                                            </label>
                                            <div class=" mission-task">
                                                <p class="text-size14 cor-gray89">{!! Theme::get('site_config')['site_employee'] ? Theme::get('site_config')['site_employee'] : '威客' !!}们先工作，再选中标作品。托管赏金后吸引更多{!! Theme::get('site_config')['site_employee'] ? Theme::get('site_config')['site_employee'] : '威客' !!}。</p>
                                            </div>
                                        </div>

                                        <div class="mission-task panel-collapse collapse @if($task['type_id'] == $type_v['id'])in active @endif" id="ck">
                                            <div class="mission-ck z-check-validform-ck">
                                                ...
                                                <div>
                                                    <input type="number" name="bounty{{$type_v['alias']}}" id="bounty" @if($task['type_id'] == $type_v['id']) value="{{$task['bounty']}}" @endif  class="mis-txt" ajaxurl="/task/checkbounty" datatype="decimal" nullmsg="请填写你的预算！" errormsg="请填写数字,最多保留两位小数！">
                                                </div>
                                                <div class="checkbox ">
                                                    <label class="checkbox-inline">
                                                        希望有多少服务商完成此任务？
                                                ...
                                                <div>
                                                    <input type="number" name="worker_num{{$type_v['alias']}}" class="mis-txt" datatype="positive"  nullmsg="请填写服务商数量！" errormsg="请填写大于0的整数！" @if($task['type_id'] == $type_v['id'])value="{{$task['worker_num']}}" @endif>
                                                ...
                                                <div class=" input-group" style="width:60%;margin-left:20px;">
                                                    ...
                                                    <div>
                                                        <input type="text" class="input-sm form-control " id="datepiker-begin" onchange="beginAt($(this))" @if($task['type_id'] == $type_v['id'] && $type == 0) value="{!! ($task['begin_at'])?date('Y年m月d日',strtotime($task['begin_at'])):date('Y年m月d日',time()) !!}" @else value="{{date('Y年m月d日',time())}}" @endif placeholder="开始时间" >
                                                    </div>
                                                    <span class="input-group-addon  date-icon ">
                                                        <i class="fa fa-exchange"></i>
                                                    </span>
                                                    <div>
                                                        <input type="text" class="input-sm form-control " id="datepiker-deadline" onchange="deadline($(this))" @if($task['type_id'] == $type_v['id'] && $type == 0) value="{!! ($task['delivery_deadline'])?date('Y年m月d日',strtotime($task['delivery_deadline'])):date('Y年m月d日',time()) !!}" @endif placeholder="截稿时间">
                                                    </div>
                                                </div>
                                                ...
                                                @endif
                                                <input name="begin_at{{$type_v['alias']}}" type="hidden" id="begin_at" datatype="*" nullmsg="请填写开始时间！" class="begin_at" @if($task['type_id'] == $type_v['id'] && $type == 0) value="{!! ($task['begin_at'])?date('Y年m月d日',strtotime($task['begin_at'])):date('Y年m月d日',time()) !!}" @else value="{{date('Y年m月d日',time())}}" @endif/>
                                                <input name="delivery_deadline{{$type_v['alias']}}" type="hidden" id="delivery_deadline" ajaxurl="/task/checkdeadline" datatype="*" nullmsg="请填写截稿时间！" class="delivery_deadline" @if($task['type_id'] == $type_v['id'] && $type == 0) value="{!! ($task['delivery_deadline'])?date('Y年m月d日',strtotime($task['delivery_deadline'])):date('Y年m月d日',time()) !!}" @endif />
                                        ...
                                        <div class="checkbox">
                                            <label class="z-check z-check-validform"
                                                ...
                                                <input  type="radio" class="ace" name="type_id"
                                                        ...
                                                        value="{{$type_v['id']}}" data-values="{{$type_v['alias']}}"/>
                                        ...
                                        <div class="mission-task panel-collapse collapse
                                            @if($task['type_id'] == $type_v['id'])in active @endif" id="ck1">
                                            <div class="mission-ck z-check-validform-ck">
                                                @if($isKee)
                                                    <div class="checkbox">
                                                        <label class="checkbox-inline">
                                                ...
                                                <div class="checkbox">
                                                    ...
                                                    <div class=" input-group" style="width:60%;margin-left:20px;">
                                                        ...
                                                        <div>
                                                            <input type="text" class="input-sm form-control "
                                                                   ...
                                                                   @if($task['type_id'] == $type_v['id'] && $type == 0) value="{!! ($task['begin_at'])?date('Y年m月d日',strtotime($task['begin_at'])):date('Y年m月d日',time()) !!}" @else value="{{date('Y年m月d日',time())}}" @endif placeholder="开始时间" >
                                                        </div>
                                                        <span class="input-group-addon  date-icon ">
                                                            <i class="fa fa-exchange"></i>
                                                        </span>
                                                        <div>
                                                            <input type="text" class="input-sm form-control " id="datepiker-deadline1" onchange="deadline($(this))" @if($task['type_id'] == $type_v['id'] && $type == 0) value="{!! ($task['delivery_deadline'])?date('Y年m月d日',strtotime($task['delivery_deadline'])):date('Y年m月d日',time()) !!}" @endif placeholder="截止时间">
                                                        </div>
                                                    </div>
                                                    @if($errors->first('begin_at'.$type_v['alias'])|| $errors->first('delivery_deadline'.$type_v['alias']) )
                                                        <p class="Validform_checktip Validform_wrong">
                                                    ...
                                                    @endif
                                                    <input name="begin_at{{$type_v['alias']}}" type="hidden" id="begin_at1" datatype="*" nullmsg="请填写开始时间！" @if($task['type_id'] == $type_v['id']) value="{!! ($task['begin_at'] && $type == 0)?date('Y年m月d日',strtotime($task['begin_at'])):date('Y年m月d日',time()) !!}" @else value="{{date('Y年m月d日',time())}}" @endif class="begin_at"/>
                                                    <input name="delivery_deadline{{$type_v['alias']}}" type="hidden" id="delivery_deadline1"
                                                           ajaxurl="/task/checkDeadlineByBid" datatype="*" nullmsg="请填写截止时间！" @if($task['type_id'] == $type_v['id'] && $type == 0) value="{!! ($task['delivery_deadline'])?date('Y年m月d日',strtotime($task['delivery_deadline'])):date('Y年m月d日',time()) !!}" @endif class="delivery_deadline"/>
                                                ...
                                                <div>
                                                    ...
                                                    <span></span> -->
                                                    <input type="number" name="bounty{{$type_v['alias']}}" id="bounty_zhaobiao"
                                                           value="{!! old('bounty') !!}"  class="mis-txt"
                                                           {{--ajaxurl="/task/checkbounty" datatype="zhaobiaobounty"
                                                           errormsg="请填写数字,最多保留两位小数！"--}}>
                                                    @if($errors->first('bounty'))
                                                        <p class="Validform_checktip Validform_wrong">
                                                            {!! $errors->first('bounty') !!}</p>
                                                    @endif
                                                    <span class="Valiform_checktip"></span>
                                                </div>
                                            </div>
                                        </div>
                                        @endif
                    ...
                    <div class="task-bt">
                        ...
                        <button class="btn btn-primary btn-blue bor-radius2 btn-big1 preservation" onclick="sluSub(1)">发布</button><a href="javascript:sluSub(2);"  class="text-size14 preview">预览任务</a><a href="javascript:sluSub(3);" class="text-size14 not_released">暂不发布</a>
                    </div>
                </div>
            </form>
        </div>
