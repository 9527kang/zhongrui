<div class="space-10 hidden-md hidden-sm hidden-xs"></div>
<div class="col-sm-12 col-left col-right">
    <div class="space-10"></div>
</div>

<div class="space-10"></div>
<!--最新任务-->
<div class="clearfix">
    <div class="col-lg-9 col-md-12 col-sm-12 col-xs-12 m-task col-left col-right">
        <div class="clearfix">
            <h4 class="pull-left text-size24 cor-gray45">最新任务</h4>
            <a class="pull-right cor-gray97 u-more" href="/task" target="_blank">More></a>
        </div>
        <div class="space-4"></div>
        <div class="g-taskleft b-border clearfix">
            <div class="space"></div>
            <ul class=" clearfix text-size14 m-homelist cor-grayC2 mg-margin col-sm-12">
                @if($task)
                    @foreach($task as $k1 => $v1)
                        <li class="col-md-4 col-sm-5 col-xs-6 g-taskItem">
                            <p class="p-space" style="margin-bottom: 6px;">
                                <a class="cor-gray51 s-hometit" href="/task/{{$v1['id']}}" target="_blank">{{$v1['title']}}</a>
                            </p>
                            <p class="p-space mg-margin">
                                <span class="cor-orange s-homewrap1 p-space" style="font-size:12px;color:#2f549f;">@if($v1['show_cash'])￥{{$v1['show_cash']}}@else ￥0 @endif</span>
                                <span class="s-homewrap1 p-space tender_style">{{$v1['name']}}</span>
                                <span class="s-homewrap1 p-space pull-right" style="font-size: 12px;">{{$v1['delivery_count']}}投标</span>
                            </p>
                        </li>
                    @endforeach
                @endif
            </ul>
        </div>
    </div>

    <!--最新动态-->
    <div class="col-lg-3  g-taskright hidden-md hidden-sm hidden-xs col-left col-right">
        <h4 class=" text-size24 cor-gray45">最新动态</h4>
        <div class="space-8"></div>
        <div class="b-border txtMarquee-top u-rightwrap1 clearfix">
            <div class="bd clearfix">
                <ul class="mg-margin  clearfix">
                    @if($active)
                        @foreach($active as $k2 => $v2)
                            <li class="u-rlistitem clearfix">
                                <p class="text-size14 s-hometit">
                                    <i class="fa fa-circle-thin cor-grayC2 text-size12"></i>
                                    <a href="/bre/serviceCaseList/{{$v2['uid']}}" class="cor-blue2f" style="color:#2f549f !important" target="_blank">{{$v2['name']}}</a>
                                    接受了任务：<a href="/task/{{$v2['task_id']}}" target="_blank" style="color:#515151 !important">{{$v2['title']}}</a>
                                </p>

                                <div class="clearfix cor-grayC2">
                                    <span class="pull-left">赏金：<span class="cor-orange" style="color:#2f549f !important">@if($v2['show_cash'])
                                                ￥{{$v2['show_cash']}}@else ￥0 @endif</span>
                                    </span>
                                    <span class="pull-right">@if(intval((time() - strtotime($v2['created_at']))/60) > 60)
                                            1小时前 @else {{intval((time() - strtotime($v2['created_at']))/60)}}
                                            分钟前 @endif
                                    </span>
                                </div>
                            </li>
                        @endforeach
                    @endif
                </ul>
            </div>
        </div>
    </div>
</div>


<div class="space-10"></div>
<!--推荐店铺-->
<div class="clearfix">
    <div class="col-sm-12 hidden-xs col-left col-right">
        @if($shop)
            <div class="clearfix">
                <h4 class="pull-left text-size24 cor-gray45">{{$recommend_shop['name']}}</h4>
                <a class="pull-right cor-gray97 u-more u-more-nopd" href="/bre/service" target="_blank">More></a>
            </div>
        @endif
        <div class="space-4"></div>
    </div>
    <div class="col-sm-12 hidden-xs">
        <div class="row">
            <ul class="clearfix mg-margin g-servicer clearfix g-serv row">
                @forelse($shop as $i => $vs)
                    @if($i >=0 && $i <=3)
                        <li class="frist_li_m1">
                            <ul class="first_ul_m1">
                                @forelse($vs as $kk => $v3)
                                    <li class="li_style_store pull-left">
                                        <div class="img_title_dre">
                                            <div class="img_box_m1">
                                                <div class="img_style_m1 pull-left">
                                                    <a href="{{$v3['url']}}">
                                                        <img src="@if(!empty($v3['recommend_pic'])){{url($v3['recommend_pic'])}}@else {{url($v3['shop_pic'])}}@endif" alt="img" width="145" height="145">
                                                    </a>
                                                </div>
                                                <div class="title_style_m1 pull-left">
                                                    <h4 class="title_content_m1" style="margin-top: 0 ;">
                                                        <a href="{{$v3['url']}}" style="font-weight: 500;">
                                                            @if(!empty($v3['shop_name']))
                                                                {{$v3['shop_name']}}
                                                            @else
                                                                这是店铺名称
                                                            @endif</a>
                                                    </h4>
                                                    <div class="icon_all_m1">
                                                        @if($v3['bank_auth'] == true)
                                                            <span class="s-servicericon bank-attestation"></span>
                                                        @else
                                                            <span class="s-servicericon bank-attestation-no"></span>
                                                        @endif
                                                        @if($v3['realname_auth'] == true)
                                                            <span class="s-servicericon cd-card-attestation"></span>
                                                        @else
                                                            <span class="s-servicericon cd-card-attestation-no"></span>
                                                        @endif
                                                        @if(isset($v3['email_status']) && $v3['email_status'] == 2)
                                                            <span class="s-servicericon email-attestation"></span>
                                                        @else
                                                            <span class="s-servicericon email-attestation-no"></span>
                                                        @endif
                                                        @if($v3['alipay_auth'] == true)
                                                            <span class="s-servicericon alipay-attestation"></span>
                                                        @else
                                                            <span class="s-servicericon alipay-attestation-no"></span>
                                                        @endif
                                                        @if($v3['enterprise_auth'] == true)
                                                            <span class="s-servicericon com-attestation"></span>
                                                        @else
                                                            <span class="s-servicericon com-attestation-no"></span>
                                                        @endif
                                                    </div>
                                                    <div class="comment_style_m1 clearfix">
                                                        <span class="pull-left comment_text_m1">好评率: <a>{{$v3['good_comment_rate']}}%</a></span>
                                                        <span class="pull-left comment_text_m1">好评数: <a>{{$v3['good_comment']}}</a></span>
                                                        <span class="pull-left comment_text_m1">累计雇佣: <a>{{$v3['serviceNum']}}</a></span>

                                                    </div>
                                                    <div class="btn_style_m1 clearfix">
                                                        <a href="{{$v3['url']}}" class="btnStyle_m1">查看详情</a>
                                                    </div>
                                                </div>
                                                <div class="pull-left dress_style_m1">
                                                    @if(isset($v3['addr']) && !empty($v3['addr']))
                                                        <span><i class="fa fa-map-marker"></i> {{$v3['addr']}}</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
                                        <div class="classtify_all_m1">
                                            <div class="box_classtify_m1">
                                                <div class="title_small_m1 pull-left">
                                                    <span>提供商品</span>
                                                </div>
                                                <div class="content_text_m1 pull-left">
                                                <span>
                                                    @if(isset($v3['success']) && !empty($v3['success']))
                                                        @foreach($v3['success'] as $kk => $vv)
                                                            <span>
                                                                    <a @if($vv['type'] == 1) href="/shop/buyGoods/{{$vv['id']}}" @elseif($vv['type'] == 2)href="/shop/buyservice/{{$vv['id']}}" @endif target="_blank">￥{{$vv['cash']}}&nbsp;{{$vv['title']}}</a>
                                                                </span>
                                                        @endforeach
                                                    @endif
                                                </span>
                                                </div>
                                            </div>
                                        </div>
                                    </li>
                                @empty
                                @endforelse
                            </ul>
                        </li>
                    @endif
                @empty
                @endforelse
            </ul>
        </div>
    </div>
</div>

<div class="clearfix col-sm-12 col-left col-right banner_style_m2">
    <div class="space-10"></div>
    <!--广告位-->
    @if(count($ad))
        <div class="for-advertise">
            <a  target="_blank" href="{{url($ad[0]['ad_url'])}}"><img src="{{url($ad[0]['ad_file'])}}" alt=""></a>
        </div>
    @endif
    <div class="space-10"></div>
</div>

<div class="space-10"></div>

<!-- 威客商城 -->
<div class="clearfix shop_box_m1">
    <div class="col-sm-12 col-left col-right" style="padding-left: 0;">
        @if($work)
            <div class="clearfix">
                <h4 class="pull-left text-size24 cor-gray45">威客商城</h4>
                <a class="pull-right cor-gray97 u-more u-more-nopd" href="/bre/shop" target="_blank">More></a>
            </div>
        @endif
        <div class="space-4"></div>
    </div>
    <ul class="clearfix mg-margin g-servicer  g-servicer-list shop_all_m1">
        @forelse($work as $k => $v)
            @if($k < 4)
                <li class="pull-left col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    <div class="col-xs-12">
                        <div class="img_show_m1">
                            <a href="{{$v['url']}}">
                                <img @if($v['recommend_pic'])src="{!! URL($v['recommend_pic']) !!}" @else src="{!! URL($v['cover']) !!}" @endif alt="shop_img" width="253" height="243">
                            </a>
                        </div>
                        <div class="shoping_title_box_m1">
                            <h4>
                                <a href="{{$v['url']}}">{{  $v['title'] }}</a>
                            </h4>
                            <div class="comment_text_m2">
                                <span class="production_style_m1">作品</span>
                                <!-- <span class="service_style_m1">服务</span> -->
                                <span class="comment_text_m1">好评率: <a>@if(!empty($v['comments_num'])){!! intval(($v['good_comment']/ $v['comments_num'])*100) !!}% @else 0% @endif</a></span>
                            </div>
                            <div class="shop_money_m1">
                                <span class="pull-left cor-orange text-size16" style="font-size:12px;color:#6c81b5;">
                                    <span class="text-size12" style="color:#6c81b5;">￥</span>
                                    {!! $v['cash'] !!}
                                </span>
                                <span class="pull-right num_people_m1">
                                    <a>@if(!empty($v['sales_num']))
                                            {!! $v['sales_num'] !!}
                                        @else
                                            0
                                        @endif
                                    </a>
                                    人购买
                                </span>
                            </div>
                        </div>
                    </div>
                </li>
            @endif
        @empty
        @endforelse
    </ul>
</div>


<div class="space-10"></div>
<!-- 成功案例 -->
<div class="clearfix success_box_m1">
    <div class="col-sm-12 col-left col-right" style="padding-left: 0;">
        @if($success)
            <div class="clearfix">
                <h4 class="pull-left text-size24 cor-gray45">{{$recommend_success['name']}}</h4>
                <a class="pull-right cor-gray97 u-more u-more-nopd" href="/task/successCase" target="_blank">More></a>
            </div>
        @endif
        <div class="space-4"></div>
    </div>
    <div class="col-sm-12">
        <div class="row">
            <ul class="pull-left success_box_ul_m1">
                @forelse($success as $k => $v)
                    @if($k < 6)
                        <li class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                            <div class="col-xs-12">
                                <div class="img_success_m1">
                                    <a href="{{$v['url']}}">
                                        <img @if($v['recommend_pic'])src="{!! URL($v['recommend_pic']) !!}" @else src="{!! URL($v['success_pic']) !!}" @endif alt="案例01" width="285" height="180">
                                    </a>
                            <span class="positionBox_m1">
                                <span class="pull-left">交易金额：￥{{$v['cash']}}</span>
                                <span class="pull-right">中标人：{{$v['bidname']}}</span>
                            </span>
                                </div>
                                <div class="text_box_m2">
                                    <span>{{$v['title']}}</span>
                                </div>
                                <b class="tier_all_m1">
                                    <a href="{{$v['url']}}">
                                        <div class="text_case01_m1">
                                            <span>{{$v['name']}}</span>
                                        </div>
                                        <div class=text_case02_m1>
                                            <span>发布者：{{$v['username']}}</span>
                                        </div>
                                    </a>
                                </b>
                            </div>
                        </li>
                    @endif
                @empty
                @endforelse
            </ul>
            @if(count($adSuccessSide))
                <div class="pull-right success_img_show_m1">
                    <a target="_blank" href="{{url($adSuccessSide[0]['ad_url'])}}">
                        <img  src="{{url($adSuccessSide[0]['ad_file'])}}" alt="案例" width="285" height="465">
                    </a>
                </div>
            @else
                <div class="pull-right success_img_show_m1">
                    <a>
                        <img src="/themes/default/assets/images/banner_slider.png" alt="案例" width="285" height="465">
                    </a>
                </div>
            @endif

        </div>
    </div>
</div>

<div class="space-10"></div>
<!-- 资讯 -->
<div class="clearfix g-info info_box_m1">
    <div class="col-sm-12 col-left col-right" style="padding-left: 0;">
        @if(!empty($article))
            <div class="clearfix">
                <h4 class="pull-left text-size24 cor-gray45">{{$recommend_article['name']}}</h4>
                <a class="pull-right cor-gray97 u-more u-more-nopd" href="/article" target="_blank">More></a>
            </div>
        @endif
        <div class="space-4"></div>
    </div>

    <div class="clearfix">
        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 col-left col-right" style="margin-bottom:20px;">
            @if(!empty($article) && is_array($article))
                <div class="f-pr">
                    <a href="{{$article[0]['url']}}" target="_blank">
                        <img src="{{url($article[0]['recommend_pic'])}}" alt="" class="img-responsive j-img">
                    </a>
                    <div class="f-prwarp">
                        <h5>
                            <a href="{{$article[0]['url']}}" target="_blank" class="cor-white">
                                {{$article[0]['title']}}
                            </a>
                        </h5>
                    </div>
                </div>
            @endif
        </div>
        @if($articleArr)
            <div class="col-lg-8 col-md-12 ">
                <div class="row">
                    @forelse($articleArr as $k => $v)
                        @if($k < 4)
                            <div class="col-md-6 col-sm-6 col-xs-12 g-infoitem f-pd clearfix col-left col-right">
                                <div class="clearfix b-border" style="border-radius:5px;">
                                    <div class="pull-left u-infowrap">
                                        <a href="{{$v['url']}}" target="_blank" class="cor-gray51">
                                            <img src="{{url($v['recommend_pic'])}}" alt="" class="j-img img-responsive pull-left">
                                        </a>
                                    </div>
                                    <div class="u-infoItm clearfix">
                                        <h4 class="text-size16 p-space">
                                            <a href="{{$v['url']}}" class="cor-gray51">
                                                {{$v['title']}}
                                            </a>
                                        </h4>
                                        <div class="space-2"></div>
                                        <p class="cor-gray97">
                                            @if(mb_strlen($v['summary']) > 45)
                                                {!! mb_substr($v['summary'],0,45,'utf-8') !!}...
                                            @else
                                                {!! $v['summary'] !!}
                                            @endif
                                        </p>
                                        <div class="space-4 hidden-sm hidden-xs"></div>
                                        <div class="clearfix text-right">
                                            <a href="{{$v['url']}}" class="cor-gray97 p-space">{{$v['cate_name']}} ·  详情</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        @endif
                    @empty
                    @endforelse
                </div>
            </div>
        @endif
    </div>
</div>
<!-- 友情链接 -->
<div class="clearfix g-lk link_box_m1" style="margin-top:30px;">
    <div class="col-sm-12 col-left col-right">
        <div class="clearfix">
            <h4 class="text-size24 cor-gray45">友情链接</h4>
        </div>
        <div class="space-4"></div>
        <div class="clearfix u-gray g-lkroll">
            <div class="clearfix">
                <a class="z-btn1 next " href="javascript:;"><i class="fa fa-angle-left text-size24"></i></a>
                <a class="z-btn2 prev" href="javascript:;" ><i class="fa fa-angle-right text-size24"></i></a>
            </div>
            <div class="bd">
                <ul class="mg-margin picList">
                    @if($friendUrl)
                        @foreach($friendUrl as $k6 => $v6)
                            <li class=" text-center u-item">
                                <div class="">
                                    <a target="_blank" href="{{url($v6['content'])}}">
                                        <img src="{{url($v6['pic'])}}" alt="kppw">
                                    </a>
                                </div>
                            </li>
                        @endforeach
                    @endif
                </ul>
            </div>
        </div>
    </div>
</div>
<div class="space-10"></div>


{!! Theme::asset()->container('custom-css')->usepath()->add('index','css/index.css') !!}
{!! Theme::asset()->container('specific-js')->usepath()->add('SuperSlide','plugins/jquery/superSlide/jquery.SuperSlide.2.1.1.js') !!}
{!! Theme::asset()->container('custom-js')->usepath()->add('homepage','js/doc/homepage.js') !!}
{!! Theme::asset()->container('specific-js')->usepath()->add('adaptive','plugins/jquery/adaptive-backgrounds/jquery.adaptive-backgrounds.js') !!}


