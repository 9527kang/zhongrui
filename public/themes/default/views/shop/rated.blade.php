	...
	<div class="col-lg-3 hidden-xs hidden-md hidden-sm col-left">
        <div class="needs-sidebar rated-bottom">
            <div class="needs-sidebar-wrap">
                <div class="wrap1">
                	<div class="img_show_m3">
                		@if(is_file($shopInfo['shop_pic']))
						 <img src="{!! $domain.'/'.$shopInfo['shop_pic'] !!}" alt=""/>
						@else
	                    <img src="{!! Theme::asset()->url('images/case_back.png') !!}" alt=""/>
					    @endif
					    <b>{{$shopInfo['shop_name']}}</b>
                	</div>
                </div>
                <div class="wrap2">
                ...
                <div class="wrap4" >
                	@if(Auth::check())
						...
						<span href="" class="shop-collectatv hov-orange hov_orange_m3">已收藏</span>
						@endif
					@else
					...
					@if(Auth::check())
						...
						<a href="javascript:;" class="ico1 shop-im hovim-orange  hov_orange_m4" data-values="{{ $shopInfo['uid'] }}"><i></i>联系我</a>
						@endif
					@else
					<a href="{!! URL('/login') !!}" class="ico1 a_link_style_m1"><i></i>联系我</a>
					@endif
                </div>
        ...
        <div class="needs-sidebar rated-bottom">
            <div class="needs-sidebar-wrap">
            	...
            	<div class="clearfix">
            		<div class="clearfix">
            			 <h5 class="pull-left text-size16 mg-margin cor-gray45">商品分类</h5>
			    ...
			    <div class="clearfix">
            		<div class="clearfix">
            			 <h5 class="pull-left text-size16 mg-margin cor-gray45">服务分类</h5>
							...
							<form action="/shop/rated/{!! $shopId !!}" method="get" id="screen_form">
								...
								<div class="col-xs-12 task-mediaAssessR pd-padding0">
									<label class="evaluate-back">
										...
										<span class="lbl screen" data-type="1"> <span class="flower4">好评</span></span>&nbsp;&nbsp;&nbsp;
									</label>
									<label class="evaluate-back">
										<!-- <input name="type" type="radio" class="ace screen" value="2" {{ (!empty($_GET['type']) && $_GET['type']==2)?'checked':'' }}> -->
										<span class="lbl screen" data-type="2"> <span class="flower5">中评</span></span>&nbsp;&nbsp;&nbsp;
									</label>
									<label>
										<!-- <input name="type" type="radio" class="ace screen" value="3" {{ (!empty($_GET['type']) && $_GET['type']==3)?'checked':'' }}> -->
										<span class="lbl screen" data-type="3"> <span class="flower6">差评</span></span>
									</label>
								</div>
							</form>
	                    </div>
	                    <div class="space"></div>
						@if(!empty($commentInfo) && $commentInfo->toArray()['data'])
			                ...
			                <div class="col-sm-1 col-xs-2">
		                        <div class="row">
		                            <div @if($cv->type == 0) class="g-valugood" @elseif($cv->type == 1) class="g-valuin" @elseif($cv->type == 2) class="g-valupoor" @endif>
		                                <img class="img-responsive img-circle" src="{!! $domain.'/'.$cv->avatar !!}"  alt="...">

										<div class="g-valuimgbg"></div>
		                            </div>
		                            <div class="space-6"></div>
		                            <p class="text-center g-valuin p-space" style="margin:0 auto;"><a href="javascript:;" class=" cor-blue2f">{!! $cv->name !!}</a></p>
		                        </div>
			                </div>
			                <div class="col-sm-11 col-xs-10 s-myborder pd-right0">
		                        <div class="clearfix">
		                            <span class=" pull-left text-muted text-size12 cor-gray87 s-myname">@if($cv->sort == 1) 商品 @elseif($cv->sort == 2) 服务 @endif ：<a class="cor-blue42" @if($cv->sort == 1) href="{!! URL('/shop/buyGoods/'.$cv->goodId) !!}" @elseif($cv->sort == 2) href="{!! URL('/shop/buyservice/'.$cv->goodId) !!}" @endif>   {!! $cv->title !!} </a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;成交价：￥{!! $cv->cash !!}</span>
		                            <a class="pull-right cor-gray87 text-size12" href="javascript:;">{!! date('Y-m-d',strtotime($cv->created_at)) !!}</a>
		                        </div>
		                        <div class="space-6"></div>
		                        <div class="p-space">
		                            <p class="cor-gray51 text-size14">{!! $cv->comment_desc !!}</p>
		                        </div>
