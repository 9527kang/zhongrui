                    ...
                    <form action="/task/createTask" method="post" id="form" enctype="multipart/form-data">
                        ...
                        <div class="row">
                                ...
                                <ul class="mg-margin">
                                    <li>
                                        ...
                                        <div id="demo" class="collapse in">
                                            ...
                                            <div class="task-bar">
                                                <p class="text-size14" style="margin-top:57px;margin-bottom:7px;">地域：</p>
                                            ...
                                            <div class="panel">
                                                ...
                                                <div class="mission-task panel-collapse collapse @if($task['type_id'] == $type_v['id'])in active @endif" id="ck">
                                                    <div class="mission-ck z-check-validform-ck">
                                                        ...
                                                        <div>
                                                            <input type="number" name="bounty{{$type_v['alias']}}" id="bounty" @if($task['type_id'] == $type_v['id'])value="{{$task['bounty']}}" @endif  class="mis-txt" ajaxurl="/task/checkbounty" datatype="decimal" nullmsg="请填写你的预算！" errormsg="请填写数子,最多保留两位小数！">
                                                        ...
                                                        <div>
                                                            <input type="text" name="worker_num{{$type_v['alias']}}" class="mis-txt" datatype="positive"  nullmsg="请填写服务商数量！" @if($task['type_id'] == $type_v['id'])value="{{$task['worker_num']}}" @endif errormsg="请填写大于0的整数！">
                                                        ...
                                                        <span class="input-group-addon date-icon" >
                                                            ...
                                                            <div>
                                                                <input type="number" class="input-sm form-control " id="datepiker-begin" onchange="beginAt($(this))" @if($task['type_id'] == $type_v['id']  && $type == 0 ) value="{!! ($task['begin_at'])?date('Y年m月d日',strtotime($task['begin_at'])):date('Y年m月d日',time()) !!}" @else value="{{date('Y年m月d日',time())}}" @endif placeholder="开始时间" >
                                                            </div>
                                                        <span class="input-group-addon  date-icon ">
                                                        ...
                                                        @endif
                                                        <input name="begin_at{{$type_v['alias']}}" type="hidden" id="begin_at"  datatype="*" nullmsg="请填写开始时间！" @if($task['type_id'] == $type_v['id'] && $type == 0) value="{!! ($task['begin_at'])?date('Y年m月d日',strtotime($task['begin_at'])):date('Y年m月d日',time()) !!}" @else value="{{date('Y年m月d日',time())}}" @endif/>
                                                        <input name="delivery_deadline{{$type_v['alias']}}" type="hidden" id="delivery_deadline" ajaxurl="/task/checkdeadline" datatype="*" nullmsg="请填写截稿时间！" @if($task['type_id'] == $type_v['id'] && $type == 0) value="{!! ($task['delivery_deadline'])?date('Y年m月d日',strtotime($task['delivery_deadline'])):date('Y年m月d日',time()) !!}" @endif />
                                                ...
                                                <div class="checkbox">
                                                    <label class="z-check z-check-validform" data-toggle="collapse" data-parent="#demol" href="#ck1">
                                                        <input  type="radio" class="ace" name="type_id" @if($task['type_id'] == $type_v['id'])checked="checked" @endif datatype="*" nullmsg="请选择交易模式" value="{{$type_v['id']}}" data-values="{{$type_v['alias']}}"/>
                                                ...
                                                <div class="mission-task panel-collapse collapse
                                                    ...
                                                    <div class="mission-ck z-check-validform-ck">
                                                        ...
                                                        <span class="input-group-addon date-icon" >
                                                            ...
                                                            <div>
                                                                <input type="text" class="input-sm form-control " id="datepiker-begin1" onchange="beginAt($(this))" @if($task['type_id'] == $type_v['id'] && $type == 0) value="{!! ($task['begin_at'])?date('Y年m月d日',strtotime($task['begin_at'])):date('Y年m月d日',time()) !!}" @else value="{{date('Y年m月d日',time())}}" @endif placeholder="开始时间" >
                                                            </div>
                                                        <span class="input-group-addon  date-icon ">
                                                            <i class="fa fa-exchange"></i>
                                                        </span>
                                                        ...
                                                        @endif
                                                        <input name="begin_at{{$type_v['alias']}}" type="hidden" id="begin_at1" datatype="*" nullmsg="请填写开始时间！" @if($task['type_id'] == $type_v['id'] && $type == 0) value="{!! ($task['begin_at'])?date('Y年m月d日',strtotime($task['begin_at'])):date('Y年m月d日',time()) !!}" @else value="{{date('Y年m月d日',time())}}" @endif class="begin_at"/>
                                                        <input type="hidden" id="delivery_deadline1"
                                                               name="delivery_deadline{{$type_v['alias']}}" ajaxurl="/task/checkDeadlineByBid"  datatype="*" nullmsg="请填写截止时间！" @if($task['type_id'] == $type_v['id'] && $type == 0) value="{!! ($task['delivery_deadline'])?date('Y年m月d日',strtotime($task['delivery_deadline'])):date('Y年m月d日',time()) !!}" @endif class="delivery_deadline"/>
                                                    <span class="time_check validform_checktip"
                                                          style="width:90%;margin-left:20px;display:none">
                                                        ...
                                                        <div>
                                                            <input type="text" name="bounty{{$type_v['alias']}}" class="mis-txt" @if($task['type_id'] == $type_v['id'] && $task['bounty'] > 0)value="{{floatval($task['bounty'])}}" @endif placeholder="若无明确预算，可不填" >
                            ...
                            <span>￥
                                ...
                                <button class="btn btn-primary btn-blue bor-radius2 btn-big1 preservation" onclick="sluSub(1)">发布</button><a href="javascript:sluSub(2);"  class="text-size14 preview">预览任务</a><a href="javascript:sluSub(3);" class="text-size14 not_releasedsss">暂不发布</a>
                            </div>
                        </div>
                    </form>
                </div>
