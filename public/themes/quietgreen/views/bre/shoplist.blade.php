...
<article>
    <div class="container">
        ...
        <div class="classify serivce-type">
            <label><i class=" fa fa-th-large"></i>&nbsp;&nbsp;&nbsp;分类:</label>
           <a href="{!! URl('bre/shop').'?'.http_build_query(array_except($merge,['page','category'])) !!}">
            ...
            @if(count($category)>8)
                ...
                <div class="col-xs-12 clearfix serivcelist-type">
                    <div class="">
                        <div class="col-md-offset-1 cor99">
                            <div class="row">
                                @foreach(array_slice($category,8,(count($category)-8)) as $v)
                                    <a  href="{!! URL('/bre/shop').'?'.http_build_query(array_merge($merge, ['category'=> $v['id']])) !!}"><span class="classify-wrap {!! (isset($merge['category']) && $merge['category']==$v['id'])?'active':'' !!}">{{ $v['name'] }}</span></a>
                                @endforeach
                            </div>
                        </div>
