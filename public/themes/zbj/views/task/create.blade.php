            ...
            <ul class="mg-margin">
                <li>
                    <a class="task-tit" href="javascript:;" data-toggle="collapse"
                       data-target="#demo">
                        <span class="ace-icon fa fa-angle-double-down" data-icon-hide="ace-icon fa fa-angle-double-down" data-icon-show="ace-icon fa fa fa-angle-double-right">
                        </span> 描述需求
                    </a>
                    <div id="demo" class="collapse in">
                        <div class="form-group task-phone clearfix task-checkip-right">
                            <p for="name" class="phone text-size14">联系手机：</p>
                            <input type="text" class="form-control task-input pull-left" id="name"  name="phone" value="{!! old('phone') !!}" placeholder="请输入手机号" datatype="m" nullmsg="请填写手机号！" errormsg="手机号错误！">
                        ...
                        <div  class="task-select-bottom task-validform-right">
                            <div id="task-select"  class="task-select" style="margin-bottom: 0px;">
                                ...
                                @foreach($hotcate as $v)
                                    <a url="{{ URl('task/getTemplate') }}" cate-id="{{ $v['id'] }}" class="chooseCate"  >
                                        {{ $v['name'] }}
                                    </a>
                                @endforeach
                                <span>
                            ...
                            <div class="task-select1 collapse" id="gd"  style="margin-bottom: 0px;">
                                <p class="text-blod text-size14">选择需要做什么：</p>
                                <div class="all_select_boxShow">
                                    <div id="selectM1">
                                        <span class="select_one">请选择</span>
                                        <b class="down_icon"></b>
                                    </div>
                                    <div class="parent_box_m1">
                                        <ul class="first_box_m1"></ul>
                                    </div>
                                </div>
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header" style="background-color:#e4e4e4;">
                                                <button type="button" class="close" data-dismiss="modal" id="model-close" aria-hidden="true">&times;</button>
                                            ...
                                            <div class="modal-body">
                                                <div id="myTabContent" class="tab-content">
                                                    ...
                                                    <div class="text-center">
                                                        <button type="button" onclick="insert_example()" class="btn btn-primary" id="insertTemplate" data-toggle="button">插入模板
                                                        </button>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div class="clearfix  Validform-wysiwyg-editor" id="editor_validform">
                            ...
                            <script id="editor"  type="text/plain" style="width:100%;height:300px;" >{!! old('description') !!}
                            </script>
                        ...
                        <div class="annex">
                            <!--文件上传-->
                            <div  class="dropzone clea>fix" id="dropzone" url="{{ URL('task/fileUpload')}}" deleteurl="{{ URL('task/fileDelet') }}">
                                <div class="fallback">
                        ...
                        <div class="panel panel-default">
                            ...
                            <div class="checkbox panel-heading">
                                <label class="z-check z-check-validform" data-toggle="collapse" data-parent="#demol" href="#ck">
                                    <input  type="radio" class="ace" name="type_id" {{ (!empty(old('xuanshang')))?'checked':'' }} datatype="*" nullmsg="请选择交易模式" value="{{$type_v['id']}}" data-values="{{$type_v['alias']}}"/>
                            ...
                            {{ (!empty(old('xuanshang')))?'in':'' }}" id="ck">
                                <div class="mission-ck z-check-validform-ck">
                                    ...
                                    <div>
                                        <input type="number" name="bounty{{$type_v['alias']}}" id="bounty" value="{!! old('bounty') !!}"  class="mis-txt" ajaxurl="/task/checkbounty" datatype="decimal" nullmsg="请填写你的预算！" errormsg="请填写数字,最多保留两位小数！">
                                    ...
                                    <div>
                                        <input type="number" name="worker_num{{$type_v['alias']}}" class="mis-txt" datatype="positive"  nullmsg="请填写服务商数量！" errormsg="请填写大于0的整数！">
                                    ...
                                    @endif
                                    <input name="begin_at{{$type_v['alias']}}" type="hidden" id="begin_at" datatype="*" nullmsg="请填写开始时间！" class="begin_at" value="{!! (old('begin_at'))?old('begin_at'):date('Y年m月d日',time()) !!}"/>
                                    <input name="delivery_deadline{{$type_v['alias']}}" type="hidden" id="delivery_deadline" ajaxurl="/task/checkdeadline" datatype="*" nullmsg="请填写截稿时间！" class="delivery_deadline" value="{!! old('delivery_deadline') !!}" />
                                    <span class="time_check validform_checktip  Validform_wrong" style="width:90%;margin-left:20px;display:none"></span>
                                </div>
                            </div>
                            @endif
                            ...
                            <div class="checkbox panel-heading">
                                <label class="z-check z-check-validform" data-toggle="collapse" data-parent="#demol" href="#ck1">
                                    <input type="radio" class="ace" name="type_id" {{ (!empty(old('zhaobiao')))?'checked':'' }} datatype="*" nullmsg="请选择交易模式" value="{{$type_v['id']}}" data-values="{{$type_v['alias']}}"/>
                            ...
                            <div class="mission-task panel-collapse collapse
                                ...
                                <div class="mission-ck z-check-validform-ck">
                                    @if($isKee)
                                        <div class="checkbox">
                                            <label class="checkbox-inline">
                                                是否接入交付台(使用<a target="_blank" href="http://www.kee.im/home">交付台</a>管理项目，可提交交付计划
                                    ...
                                    <div class="input-group" style="width:60%;margin-left:20px;">
                                        ...
                                        <div>
                                            <input type="text" class="input-sm form-control " id="datepiker-begin1" onchange="beginAt($(this))" value="{!! (old('begin_at'))?old('begin_at'):date('Y年m月d日',time()) !!}" placeholder="开始时间" >
                                        </div>
                                        <span class="input-group-addon  date-icon ">
                                            <i class="fa fa-exchange"></i>
                                        </span>
                                        <div>
                                            <input type="text" class="input-sm form-control " id="datepiker-deadline1" onchange="deadline($(this))" value="{!! old('delivery_deadline') !!}" placeholder="截止时间">
                                        </div>
                                    </div>
                                    @if($errors->first('start')|| $errors->first('delivery_deadline') )
                                        <p class="Validform_checktip Validform_wrong">
                                    ...
                                    @endif
                                    <input name="begin_at{{$type_v['alias']}}" type="hidden" id="begin_at1" datatype="*" nullmsg="请填写开始时间！" class="begin_at" value="{!! (old('begin_at'))?old('begin_at'):date('Y年m月d日',time()) !!}"/>
                                    <input name="delivery_deadline{{$type_v['alias']}}" type="hidden" id="delivery_deadline1" class="delivery_deadline" ajaxurl="/task/checkDeadlineByBid" datatype="*" nullmsg="请填写截止时间！" value="{!! old('delivery_deadline') !!}" />
                                    <span class="time_check validform_checktip  Validform_wrong" style="width:90%;margin-left:20px;display:none"></span>
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="bounty{{$type_v['alias']}}" id="bounty_zhaobiao" value="{!! old('bounty') !!}"  class="mis-txt">
                                        @if($errors->first('bounty'))
                                            <p class="Validform_checktip Validform_wrong">
                                                {!! $errors->first('bounty') !!}</p>
                                        @endif
                                ...
                                <li class="clearfix">
                                    <div class="pull-left">
                                        <div class="checkbox pull-left">
                                            <label>
                                                <input type="checkbox" name="product[]" {{ (!empty(old('product')) && in_array($v['id'],old('product')))?'checked':'' }} class="taskservice" price="{{ number_format($v['price']*$off/100,2) }}" value={{ $v['id'] }}><span>{{ substr($v['title'],3,3) }}</span>
                                            </label>
                                        </div>
                                        <div class="pull-left">
                                    ...
                                    <div class="pull-right vat-txt">
                                        ￥{{ number_format($v['price']*$off/100,2) }}
                                    </div>
                                </li>
                            @endforeach
                            <li class="clearfix">
            ...
            <div style="display:none;" id="service-box">
                ...
                @foreach($service as $v)
                    <p class="bt-pd" style="display:none;" id="service-{{ $v['id'] }}">
                        {{ $v['title'] }}：<span>￥{{ number_format($v['price']*$off/100,2) }}
                        </span>
                    </p>
                @endforeach
            </div>
...
<div id="task_cate_create" data-values="{{json_encode($category_all)}}"></div>
