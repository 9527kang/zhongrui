...
class RecommendController extends ManageController
{
    ...
    public function addRecommend(Request $request){
        ...
        $data['start_time'] = (isset($data['start_time']) && !empty($data['start_time']))? preg_replace('/([\x80-\xff]*)/i', '', $request->get('start_time')) : '';
        $data['end_time'] = (isset($data['end_time']) && !empty($data['end_time']))? preg_replace('/([\x80-\xff]*)/i', '', $request->get('end_time')) : '';
        $newData = [
    ...
    public function getReInfo(Request $request){
        ...
        switch($type){
            ...
            case 'article':
                ...
                break;
            case 'task':
                $userInfo = TaskModel::select('task.id','task.title');
                $recommended = RecommendModel::getRecommended($type);
                if($recommended != false){
                    $userInfo = $userInfo->whereNotIn('task.id',$recommended);
                }
                $userInfo = $userInfo->where('task.status','>',2)
