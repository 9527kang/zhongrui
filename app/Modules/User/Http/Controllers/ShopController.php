...
class ShopController extends UserCenterController
{
    ...
    public function getShop(Request $request)
    {
        ...
        $userMore = UserMoreModel::getUserMoreByUid($uid);
        $num = 3;
        if($userMore && $userMore['skill_tags_num'] > 0){
            $num = $userMore['skill_tags_num'];
        }
    ...
    public function vippaylog($id)
    {
        ...
        $privileges = PrivilegesModel::withTrashed()->whereIn('privileges.id', $arrPrivilege)->where('package_privileges.package_id', $package->id)->leftJoin('package_privileges','package_privileges.privileges_id','=','privileges.id')->get(['privileges.title', 'privileges.desc','package_privileges.rule']);
