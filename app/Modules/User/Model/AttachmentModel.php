...
class AttachmentModel extends Model
{
    ...
    static function getAttachmentHtml($data)
    {
        $attachmentHtml = '';
        if(count($data)){
            $attachmentHtml .= '<p class="atta-'.$data['id'].'"><input type="hidden" name="file_ids[]" value="'.$data['id'].'" /><a href="/manage/fileDown/'.$data['id'].'" target="_blank">'.$data['name'].'</a><span rel="'.$data['id'].'" onclick="delFile(this)"  data-toggle="modal" data-target="#fileDelete">删除</span>';
        }
